...
async function closePopupDestacadosIfExists(page) {
...
async function selectSeason(season, p) {
  ...
  const options = await p.$$(".temporadas-box-container select option");
  const option = await (async () => {
    for (let o = 0; o < options.length; o++) {
      const opt = options[o];
      try {
        const text = await opt.evaluate((e) => {
          return e.innerText;
        });
  ...
  const opt_value = await option.evaluate((elem) => elem.value);
  await p.select(".temporadas-box-container select", opt_value);
}
...
describe("<BuscadorHomeMobile>: Operation Type Selector", () => {
  beforeAll(async () => {
    ...
    test(
      ...
      async (done) => {
        ...
        const options = await page.$$(
          ".operation-type-container select option"
        );
        ...
        option = await (async () => {
          for (let o = 0; o < options.length; o++) {
            const opt = options[o];
            try {
              const textOption = await opt.evaluate((e) => e.innerText);

              if (textOption.toUpperCase() === op.operation.toUpperCase()) {
                return opt;
              }
            } catch (e) {}
          }
        ...
        if (option != null) {
          ...
          const option_value = await (
            await option.getProperty("value")
          ).jsonValue();

          await p.select(".operation-type-container select", option_value);
...
describe("<BuscadorHome>: Property Type Selector", () => {
  beforeAll(async () => {
    await page.goto(appUrlBase);
    await closePopupDestacadosIfExists(page);
    await selectOperationType("Venta", page);
  }, 150000);
  ...
  function checkIfInSelectorPropertyType(type) {
    test(
      ...
      async (done) => {
        ...
        await p.click(".property-type-container", { timeout: 3000 });
        const options = await page.$$(".property-type-container select option");
    ...
    test(
      ...
      async (done) => {
        ...
        for (let i = 0; i < types.length; i++) {
          ...
          if (option != null) {
            const isChecked = await option.evaluate((e) => e.selected);
  ...
  const flows = [
    ...
    },
    {
      ...
      expect: {
        status: "OK",
        assert:
          "/alquiler-temporal/inmuebles/rocha/la-paloma?&fechaDesde=2020-04-15&fechaHasta=2020-04-18",
      },
    },
    {
      flow: [
    ...
    },
    {
      ...
      expect: {
        status: "OK",
        assert: "/alquiler-temporal/inmuebles/rocha/la-paloma?&temporada=santa",
      },
    },
    {
      flow: [
